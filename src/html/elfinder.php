...
$session->start();
$login = $container->getSessionComponent("LoginAccountAdmin");
if ($login == null || ! $login->isAuthed()) {
    echo 'unauthorized.';
    exit;
}
session_write_close();
...
$opts = array(
	'root'            => HTML_DIR .'/files',                       // path to root directory
	'URL'             => getBasePath() .'/files/', // root directory URL
	...
	'logger'       => new elFinderLogger(),         // object logger
...
function getBasePath() {
    $base = str_replace("/elfinder.php", "", Teeple_Util::getScriptName());
    return $base;
}
