...
class Image {
    ...
    var $type;
    var $rotate;
    var $output_ext = array('jpeg' => 'jpg', 'png' => 'png', 'gif' => 'gif');
    var $output_type;
    ...
    function save($show=false) {
        ...
        if($show) @header('Content-Type: image/'.($this->output_type ? $this->output_type : $this->type));
        ...
        if ($this->output_type) {
            ...
            $this->ext = $this->output_ext[$this->output_type];
        }
        
        $name = $show ? null: $this->dir.DIRECTORY_SEPARATOR.$this->name.'.'.$this->ext;
        if($this->type=='jpeg') imagejpeg($new_image, $name, $this->quality);
        if($this->type=='png') $show ? imagepng($new_image) : imagepng($new_image, $name);
        if($this->type=='gif') $show ? imagegif($new_image) : imagegif($new_image, $name);
